...
namespace FoodFromHeart
{
    public partial class Form11 : Form
    {
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "select * from Volunteer_Table";
        ...
        private void bunifuButton3_Click(object sender, EventArgs e)
        {
            ...
            string query = "update Volunteer_Table  set name=@name,email=@email,contact_number=@contact_number,password=@password,address=@address,picture=@picture where nid=@nid";
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "delete from Volunteer_Table where nid=@nid";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@nid", bunifuTextBox4.Text);
            con.Open();
            int a = cmd.ExecuteNonQuery();//0 1
